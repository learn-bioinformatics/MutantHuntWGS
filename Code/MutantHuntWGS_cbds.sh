...
export MODULEPATH="/hpc-prj/cbds/module:$MODULEPATH"

module load snpeff/5.1 # make home-built snpEff references available
...
#GENOME_INDEX="/hpc-prj/cbds/share/refs/saccharomyces_cerevisiae/ncbi/index/$GENOME"
...
# Make our own ploidy file
THIS_GENOME_PLOIDY_FILE="${SNPEFF_GENOME}_ploidy.txt"

# Make sure there isn't an already existing file
rm -f $THIS_GENOME_PLOIDY_FILE

cat $GENOME_FASTA `# Stream FASTA file` \
    | grep '>' `# select refline` \
    | sed 's/>//' `# remove > symbol` \
    | cut -f1 -d' ' `# select just name` \
    | parallel -k -j 1 'echo -e "{}\t1\t999999999\tM\t1"' \
    > $THIS_GENOME_PLOIDY_FILE
...
then
	...
	do
		...
		bcftools call \
            ...
            --ploidy-file "$THIS_GENOME_PLOIDY_FILE" \
...
then
	...
	do
		...
		then
			...
			snpEff ann -v $SNPEFF_GENOME "$VCF_FILE" > "$OUTPUT_FILE"/SNPeff_Output/"$VCF_NAME"/SNPeff_Annotations.vcf
	...
	do
		...
		then
			...
			##Run SIFT
			java -Xmx64G -jar $script_dir/SIFT4G_Annotator.jar -c -i "$VCF_FILE"  -d /Main/EF4.74 -r "$OUTPUT_FILE"/SIFT_Output/"$VCF_NAME"
